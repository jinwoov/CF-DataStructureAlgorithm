...
const detectTicTacToeWin = (board) => {
  ...
  let newArr = [row1, row2, row3, col1, col2, col3];
  let finalArr = helper(newArr)
  console.log(finalArr)
  for (let i =0; i < finalArr.length; i++) {
    if(finalArr[i].every(value => value === 'O'|| value === 'X')) {
      return true
    } else {
        return false
    }
...
function helper(arr) {
  ...
  for (let i = 0; i<arr[0].length ; i++) {
    diag2.push(arr[5-i][i])
    diag1.push(arr[i][i])
  }
  console.log('this is diagnol', diag2)
  console.log('this is diagnol', diag1)
  arr.push(diag1)
  arr.push(diag2)
  return arr
}
